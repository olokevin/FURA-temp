...
# cfg=$(sed -n "$SLURM_ARRAY_TASK_ID"p ${SRC_DIR}/bash_scripts/slurm_config_lora_math.txt)
...
run_name="${run_name:-${adapter_name}-lr_${lr}-rank_${lora_r}-alpha_${lora_alpha}-seed_${seed}}"
...
bash ./bash_scripts/eval_math_lora.sh \
    CKPT="$OUTPUT" \
    adapter_name="${adapter_name}" \
    base_model="${MODEL}" \
    wandb_project="${wandb_project}" \
