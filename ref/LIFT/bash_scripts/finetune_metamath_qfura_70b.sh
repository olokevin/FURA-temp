#!/bin/bash
# qfura on Llama-3-70B / MetaMathQA-100K, single H100 (94 GB).
#
# Mirrors the PiSSA QPiSSA recipe (lr=2e-5, batch=1×128, seq=512, 1 epoch on
# 100K examples) but uses qfura's BTT decomposition rather than PiSSA's
# rank-r SVD adapter. Defaults match the project-wide qfura defaults from
# CLAUDE.md: rank=full, decomp=output_one_block, train_position=small,
# s_merged_to=keep_trainable.
#
# Single-GPU 70B requires --load_strategy=layer_stream so the model never
# fully materialises in bf16 on GPU at any point.
#
# Override env vars: MAX_STEPS, lr, lora_rank (n/a for qfura), seed,
...
prompt_style="${prompt_style:-lift}"
...
wandb_project="${wandb_project:-qfura-metamath-${model_tag}}"
...
OUTPUT="${OUTPUT:-${OUTPUT_SRC_DIR}/metamath100k/${MODEL}/qfura-layout_${quant_block_layout}-decomp_${decomp_mode}_smerge_${s_merged_to}-bs${per_device_train_batch_size}x${gradient_accumulation_steps}-lr_${lr}-seed_${seed}}"
