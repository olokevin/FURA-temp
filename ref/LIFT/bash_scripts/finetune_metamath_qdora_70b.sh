#!/bin/bash
# qdora on Llama-3-70B / MetaMathQA-100K, single H100 (94 GB).
#
# Mirrors the QPiSSA paper recipe (lr=2e-5, batch=1×128, seq=512, 1 epoch on
# 100K examples) but uses NF4 base + DoRA adapters (rank 64) on top.
# Defaults align with the project-wide fura/qfura comparison: PiSSA prompt,
# fp32 adapter, fp32 AdamW so head-to-head qdora vs qfura is apples-to-apples.
#
...
wandb_project="${wandb_project:-qdora-metamath-${model_tag}}"
...
OUTPUT="${OUTPUT:-${OUTPUT_SRC_DIR}/metamath100k/${MODEL}/qdora-impl_${qdora_impl}-r_${lora_r}-alpha_${lora_alpha}-bs${per_device_train_batch_size}x${gradient_accumulation_steps}-lr_${lr}-seed_${seed}}"
