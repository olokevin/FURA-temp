...
# SVFT paper (Lingam et al., NeurIPS 2024): recommended Banded variant, target
# U/D/O/G on LLaMA-3-8B, commonsense LR 1e-3 (Table 17, SVFT_B).
svft_pattern="${svft_pattern:-banded}"
svft_off_diag="${svft_off_diag:-8}"
svft_num_coeffs="${svft_num_coeffs:-0}"
trainable_type="${trainable_type:-udog}"
lr="${lr:-1e-3}"
seed="${seed:-43}"
...
OUTPUT="${OUTPUT:-${OUTPUT_SRC_DIR}/commonsense/${MODEL}/svft-${svft_pattern}_d${svft_off_diag}-lr_${lr}-type_${trainable_type}-ep${num_train_epochs}-seed_${seed}}"
...
accelerate launch \
    ...
    --load_last_model \
    --svft_pattern ${svft_pattern} \
    --svft_off_diag ${svft_off_diag} \
    --svft_num_coeffs ${svft_num_coeffs} \
