...
| :---------- | ----------: | --------------: | --------------: | --------------: | --------------: | --------------: | --------------: | --------------: | --------------: | --------------: |
| Full FT     |      100.00 |           75.40 |           88.00 |           81.80 |           96.50 |           89.30 |           93.10 |           83.00 |           86.00 |           86.64 |
| FuRA (full) |       87.10 |           76.10 | **90.80** |           82.90 | **96.80** |           89.30 | **94.40** | **85.20** |           88.80 | **88.04** |
...
## LR Sweep — 1-Epoch Quick Eval (LLaMA-3-8B, BoolQ + OBQA only)

Quick LR search to identify the best LR per method on Meta-Llama-3-8B before committing to 3-epoch full runs.

- **Training**: 1 epoch on `commonsense_170k` (10645 optimization steps, bsz=8×2=16, max_seq=2048, seed=43)
- **Eval**: BoolQ + OpenBookQA only (faster than the full 8-task sweep)
- **Methods**: `lora` (rank=64, α=128, target_modules=`q_proj k_proj v_proj up_proj down_proj`); `blocktt`/fura (rank=full, decomp_mode=`output_one_block`, train_position=`small`, s_merged_to=`keep_trainable`, no calibration)
- **Run roots**:
  - lora → `/data/yequan/fura/lift/commonsense/meta-llama/Meta-Llama-3-8B/lora-lr_${lr}-rank_64-1ep-seed_43/last/`
  - blocktt → `/data/yequan/fura/lift/commonsense/meta-llama/Meta-Llama-3-8B/blocktt-calib_none-lr_${lr}-decomp_output_one_block_pos_small_smerge_keep_trainable-1ep-seed_43/last/`
- **Eval logs**: `<run_root>/commonsense/{boolq,openbookqa}/eval.log` — score on the `Result XX.X` line
- **Launch logs**:
